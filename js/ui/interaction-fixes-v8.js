(function(){
  ...
  function stabilizeFeatureControls(){
    $$('#featuresPage [data-feature-choice]').forEach(el=>{el.dataset.builderChoice=el.dataset.featureChoice;delete el.dataset.featureChoice});
  }
  ...
  function removeStartingEquipment(){$$('#builderBody .builder-block h3').forEach(h=>{if(/Starting Equipment/i.test(h.textContent||''))h.closest('.builder-block')?.remove()})}
  ...
  function patchAll(){if(patching)return;patching=true;observer?.disconnect();try{stabilizeFeatureControls();patchBuilder();patchTempHp();patchMoney()}finally{patching=false;observe()}}
