(function(){
  ...
  const observedSelectors=['#characterPage','#skillsPage','#featuresPage','#relicsPage'];
  ...
  function observe(){observedSelectors.forEach(sel=>{const el=$(sel);if(el)observer.observe(el,{childList:true,subtree:true})})}
  ...
  function setFlag(flag,mode,locked=false,sources=[]){
    if(!flag)return;const normalized=mode==='advantage'?'advantage':mode==='disadvantage'?'disadvantage':'normal',label=labelFor(normalized),sourceText=sources.join(', '),sig=`${normalized}|${locked?'1':'0'}|${sourceText}`;
    if(flag.dataset.renderSig===sig&&flag.textContent===label)return;
    flag.dataset.renderSig=sig;flag.classList.remove('normal','advantage','disadvantage','locked');flag.classList.add(normalized);if(locked)flag.classList.add('locked');
    if(flag.textContent!==label)flag.textContent=label;flag.dataset.mode=normalized;flag.dataset.locked=locked?'1':'0';flag.title=locked?`Fixed by: ${sourceText}`:'Click to cycle Advantage / Disadvantage';flag.setAttribute('aria-label',locked?`${normalized}. Fixed by ${sourceText}`:normalized);
  }
  ...
  function patchFeatureTrackers(){
    (T.features||[]).filter(f=>Number(f.uses)>0).forEach(f=>{
      ...
      if(!control){control=document.createElement('div');control.className='feature-use-control';detail.appendChild(control)}control.dataset.sig=sig;if(control.innerHTML!==html)control.innerHTML=html;
    });
  }
  ...
  function patchCloakChoice(){
    const raw=selectedRelic('cloak-pilgrim-worlds');if(!raw||typeof raw==='string')return;const card=[...document.querySelectorAll('#relicsPage .relic-card')].find(c=>c.querySelector('h3')?.textContent.trim()==='Plášť poutníka mezi světy');if(!card)return;
    const value=raw.choiceValue||'',sig=value||'none';let box=card.querySelector('.relic-choice-control');if(box?.dataset.sig===sig)return;const html=`<label><span>Odolnost po Dlouhém odpočinku</span><select data-relic-choice="cloak-pilgrim-worlds"><option value="">Choose…</option>${DAMAGE_CHOICES.map(x=>`<option value="${x}" ${x===value?'selected':''}>${x}</option>`).join('')}</select></label>`;
    if(!box){box=document.createElement('div');box.className='relic-choice-control';card.querySelector('.detail-actions')?.before(box)}box.dataset.sig=sig;if(box.innerHTML!==html)box.innerHTML=html;
  }
  function patchCloakDefense(){const raw=selectedRelic('cloak-pilgrim-worlds');if(!raw||typeof raw==='string'||!raw.prepared)return;const value=raw.choiceValue||'';$$('#characterPage .chip.brass').forEach(el=>{if(el.textContent.trim()==='Resistances: selected Cloak damage type'){const next=value?`Resistances: ${value}`:'Resistances: choose Cloak damage type';if(el.textContent!==next)el.textContent=next}})}
  ...
  function patchAll(){if(patching)return;patching=true;observer.disconnect();try{patchFeatureTrackers();patchCloakChoice();patchCloakDefense();patchSaveFlags();patchInitiativeFlag();patchSkillFlags();patchUniqueChoices();localizeTreasureHunterText()}finally{patching=false;observe()}}
  ...
  document.addEventListener('DOMContentLoaded',()=>{observe();patchAll()});
})();