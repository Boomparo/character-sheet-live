(function () {
  ...
  const PAGE_DEFINITIONS = [
    ...
    ['relicsPage', 'RELICS', 'relics'], ['spellsPage', 'SPELLS', 'spells'], ['gearPage', 'GEAR'], ['npcsPage', 'NPCs'], ['bioPage', 'BIO']
  ...
  function ensureDialogs() {
    ...
    document.body.insertAdjacentHTML('beforeend', `
      ...
      <dialog id="importDialog" class="sheet-dialog import-dialog"><form method="dialog"><div class="dialog-head"><strong>Import Character Data</strong><button value="cancel" class="icon-btn">×</button></div><label class="json-file-picker">Choose a .json file<input id="importFile" type="file" accept="application/json,.json"></label><div class="dialog-divider"><span>OR PASTE JSON</span></div><textarea id="importText" rows="12" spellcheck="false" placeholder="Paste complete character JSON…"></textarea><div id="importReport" class="import-report">A single character is added as a new profile. Existing characters stay untouched.</div><menu><button value="cancel" class="ghost">Cancel</button><button type="button" id="applyImport" class="primary">Import pasted JSON</button></menu></form></dialog>
  ...
  function openItemEditor(id = '') {
    ...
    $('#itemEditLibraryId').value = item?.libraryId || '';
    $('#itemEditSaveHomebrew').checked = !item || !!item?.userHomebrew;
  ...
  function applyImportText(text) {
    ...
    if (parsed.kind === 'roster') {
      ...
      if (parsed.data.homebrew) Homebrew?.importAll(parsed.data.homebrew, 'merge');
    } else {
  ...
  function onClick(event) {
    ...
    if (button.hasAttribute('data-export-roster')) { downloadJson('character-sheet-v10-all.json', { ...(Roster?.exportAll() || { current: state() }), homebrew:Homebrew?.exportAll?.() || null }); return; }
  ...
  function onSubmit(event) {
    ...
    if (form.id === 'itemEditForm') {
      ...
      if ($('#itemEditSaveHomebrew').checked && Homebrew) {
        const shared = Homebrew.saveItem({ ...values, libraryId: $('#itemEditLibraryId').value || undefined, description: values.notes, source: 'My Homebrew' });
        if (shared) Object.assign(values, { libraryId: shared.libraryId, userHomebrew: true, homebrew: true, source: shared.source });
      }
