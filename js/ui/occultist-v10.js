(function () {
  ...
  const knownSpells = source => O.spells.filter(spell => spellAvailable(spell, source) && selectedCantrip(spell, source));
  const preparedCount = source => classState(source).spells.filter(entry => entry.prepared && Number(entry.level ?? O.spells.find(spell => spell.id === (entry.id || entry.libraryId))?.level) > 0).length;
  ...
  function renderSciences(source) {
    ...
    const sciences = Object.entries(O.sciences).map(([key, science]) => {
      ...
      const levels = science.levels.slice(0, current).map((level, index) => `<details ${index + 1 === current ? 'open' : ''}><summary><span>Tier ${index + 1}</span><b>${esc(level.title)}</b></summary><div>${level.items.map(item => `<p><strong>${esc(item[0])}</strong><span>${esc(item[1])}</span></p>`).join('')}</div></details>`).join('');
    ...
    const spells = O.spells.filter(spell => spellAvailable(spell, source)).map(spell => {
      const entry = spellEntry(spell, source), isPrepared = !spell.level || !!entry?.prepared, cantripSelected = selectedCantrip(spell, source);
      return `<article class="occ-spell ${isPrepared && cantripSelected ? 'prepared' : ''}"><div><small>${spell.level ? `LEVEL ${spell.level}` : 'CANTRIP'} · ${esc(spell.science)}</small><strong>${esc(spell.name)}</strong><span>${esc(spell.time)} · ${esc(spell.range)}</span></div><p>${nl(entry?.note || entry?.desc || spell.desc || '')}</p>${spell.level ? `<button type="button" class="small-btn ${isPrepared ? 'primary' : ''}" data-occult-prepare="${spell.id}">${isPrepared ? 'PREPARED' : 'PREPARE'}</button>` : `<span class="chip ${cantripSelected ? 'brass' : ''}">${cantripSelected ? 'KNOWN' : 'ALTERNATIVE'}</span>`}</article>`;
    }).join('');
    page.innerHTML = `<div class="page-intro"><div><span class="eyebrow">OCCULTIST</span><h1>SCIENCES</h1><p>Knowledge, daily preparation and spell slots</p></div></div><section class="section occ-science-overview"><div class="section-head"><h2>Daily Practice</h2><span class="eyebrow">${preparedCount(source)}/${progress(source).prepared} PREPARED</span></div><div class="occ-slot-grid">${resourceBar(source)}</div><div class="occ-knowledge"><span><small>KNOWLEDGE SPENT</small><b class="${spent > total ? 'danger-text' : ''}">${spent}/${total}</b></span><button type="button" class="small-btn" data-occult-dawn>Mark dawn preparation complete</button><button type="button" class="small-btn" data-occult-energy>Restore slot with HP</button></div></section><section class="section"><div class="section-head"><h2>Occult Sciences</h2><small>Tier costs are cumulative.</small></div><div class="occ-science-grid">${sciences}</div></section><section class="section"><div class="section-head"><h2>Spellbook</h2><span class="eyebrow">DC ${D.spellDC(source)} · ATTACK ${signed(D.spellAttack(source))}</span></div><div class="occ-spell-grid">${spells || '<div class="empty">Study a science to unlock its spells.</div>'}</div></section>`;
  }
  ...
  function render() {
    if (!active()) return;
    const source = S.get();
    patchHudAndPages(source); patchCharacter(source); renderActions(source); renderFeatures(source); renderSciences(source);
  }
  ...
  document.addEventListener('submit', event => { if (active() && event.target.id === 'builderForm' && $('#occBuilderName')) saveBuilder(event); }, true);
  ...
  ensureClassDialog(); setTimeout(render, 0);
